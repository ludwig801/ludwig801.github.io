$(function () {
	...
	let begin = parseInt($('.timeline').attr('begin'));
	let end = parseInt($('.timeline').attr('end'));
	let length = Math.abs(end - begin);

	$('.timeline .timeline-entry').each(function () {

		let entryBegin = parseInt($(this).attr('begin'));
		let entryEnd = parseInt($(this).attr('end'));
		let entryBeginOffset = (Math.max(0, entryBegin - 0.1) - begin) / length;
		let entryEndOffset = (end - Math.min(entryEnd + 0.1)) / length;

		$(this).css('padding-left', `${entryBeginOffset * 100}%`);
		$(this).css('padding-right', `${entryEndOffset * 100}%`);

		let $bar = $(`<button type="button" class="btn timeline-bar" data-toggle="tooltip" data-placement="top" title="${entryBegin} - ${entryEnd}"></button>`).tooltip();

		$(this).append($bar);

	});

	$('.timeline-bar').on('focus pointerenter', function () {

		$(this).closest('.timeline').find('.timeline-descriptions *').toggleClass('active', false);

		let targetSelector = $(this).closest('.timeline-entry').attr('target');

		$(this).closest('.timeline').find(`${targetSelector}`).toggleClass('active', true);

	});

	$('.timeline-bar').on('blur pointerleave', function () {
